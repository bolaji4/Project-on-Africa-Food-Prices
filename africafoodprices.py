...
def get_data():
    ...
    cols_to_drop = ['Unnamed: 0','country_id','state_id',
                'currency_id','pt_id',
                'mp_commoditysource']
    df = df.drop(columns=cols_to_drop)
    new_names = {'country': 'country',
            'state': 'state',
            'market_id': 'mkt_id',
            'market': 'mkt_name',
            'produce_id': 'pd_id',
...
with st.container():
    ...
    # we build a pivot table
                pvt = pd.pivot_table(data, index=['country','state','mkt_name','product','year_recorded',],values=['price_paid'], aggfunc='mean')
    ...
    # selected product
                ...
                pvt_df = pvt_df[pvt_df["product"] == selected_product]
    #line chart
                chart = alt.Chart(pvt_df).mark_line().encode(
                x='year_recorded', y='price_paid', tooltip=['mkt_name','price_paid'])
                st.write(f"### Price Chart {selected_product} in {selected_country}")
